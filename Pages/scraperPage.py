...
class ScraperPage(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        self.folder = folder
        scraper_files = [fname for fname in os.listdir(folder) if fname.endswith(".py")]

       
        self.dropdown_var = ctk.StringVar()
        self.dropdown = ctk.CTkOptionMenu(self, values=scraper_files, variable=self.dropdown_var)
        if scraper_files:
            self.dropdown_var.set(scraper_files[0])
        self.dropdown.pack(pady=10)

       
        ctk.CTkButton(self, text="Run Scraper", command=self.run_scraper).pack(pady=10)
        ctk.CTkButton(self, text="Stop Scraper", fg_color="red", hover_color="#8b0000", command=self.stop_scraper).pack(pady=10)
